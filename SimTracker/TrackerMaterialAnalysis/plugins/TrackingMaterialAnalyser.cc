...
TrackingMaterialAnalyser::TrackingMaterialAnalyser(const edm::ParameterSet& iPSet)
{
  ...
  if (m_saveSummaryPlot)
    ...
    //    m_plotter               = new TrackingMaterialPlotter( 550., 350., 10 );      // 10x10 points per cm2, extension for HGCal
    m_plotter               = new TrackingMaterialPlotter( 1200., 350., 10 );      // 10x10 points per cm2, extension for HGCal
  else
    m_plotter               = nullptr;
}
