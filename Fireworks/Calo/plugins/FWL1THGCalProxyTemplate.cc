...
class FWL1THGCalProxyTemplate : public FWSimpleProxyBuilderTemplate<T> {
 protected:
  ...
  std::unordered_set<unsigned> getCellsFromTriggerCell(
      const unsigned trigger_cell_id) const {
    constexpr unsigned hSc_triggercell_size_ = 2;
    // constexpr unsigned hSc_module_size_ = 12; // in TC units (144 TC / panel
    // = 36 e-links)
    ...
    if (det == DetId::HGCalHSc) {
      ...
      int ieta0 =
          (trigger_cell_sc_id.ietaAbs() - 1) * hSc_triggercell_size_ + 1;
      ...
      for (int ietaAbs = ieta0; ietaAbs < ieta0 + (int)hSc_triggercell_size_;
        ...
        for (int iphi = iphi0; iphi < iphi0 + (int)hSc_triggercell_size_;
             iphi++) {
          unsigned cell_id =
              HGCScintillatorDetId(trigger_cell_sc_id.type(),
                                   trigger_cell_sc_id.layer(), ieta, iphi);
#if 0
    ...
    // Silicon
      ...
      if (subdet == HGCalTriggerSubdetector::HGCalEETrigger ||
        ...
        DetId::Detector cell_det =
            (subdet == HGCalTriggerSubdetector::HGCalEETrigger
                 ? DetId::HGCalEE
                 : DetId::HGCalHSi);
