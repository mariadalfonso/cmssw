...
void MahiFit::doFit(std::array<float, 3>& correctedOutput, int nbx) const {
  ...
  for (unsigned int iBX = 0; iBX < nnlsWork_.nPulseTot; ++iBX) {
    ...
    if (offset == pedestalBX_) {
      ...
      pulseCovArray[iBX].setZero(nnlsWork_.tsSize, nnlsWork_.tsSize);
      updatePulseShape(
		       nnlsWork_.amplitudes.coeff(nnlsWork_.tsOffset + offset), pulseCovArray[iBX] , offset, iBX );
    }
  }

  const float chiSq = minimize(pulseCovArray);
...
const float MahiFit::minimize(const std::array<SampleMatrix, MaxPVSize> & pulseCovArray) const {
  ...
  for (int iter = 1; iter < nMaxItersMin_; ++iter) {
    updateCov(invCovMat, pulseCovArray);
...
void MahiFit::updatePulseShape(const float itQ,
  ...
  pulseShape = FullSampleVector::Zero(nnlsWork_.tsSize + nnlsWork_.maxoffset + nnlsWork_.bxOffset);
  if (calculateArrivalTime_)
    pulseDeriv = FullSampleVector::Zero(nnlsWork_.tsSize + nnlsWork_.maxoffset + nnlsWork_.bxOffset);
  pulseCov = FullSampleMatrix::Zero(nnlsWork_.tsSize + nnlsWork_.maxoffset + nnlsWork_.bxOffset,
  ...
  auto const start = nnlsWork_.maxoffset - offset;
  nnlsWork_.pulseMat.col(iBX)  = pulseShape.segment(start, nnlsWork_.tsSize);
  if (calculateArrivalTime_)
    nnlsWork_.pulseDerivMat.col(iBX)  = pulseDeriv.segment(start, nnlsWork_.tsSize);
  pulseCovMatrix  = pulseCov.block(start, start, nnlsWork_.tsSize, nnlsWork_.tsSize);


}

void MahiFit::updateCov(const SampleMatrix& samplecov, const std::array<SampleMatrix, MaxPVSize> & pulseCovArray) const {
  ...
  for (unsigned int iBX = 0; iBX < nnlsWork_.nPulseTot; ++iBX) {
    ...
    // negative TS : pulseCovArray is null, reset also the noiseMatrix
    else if ( nnlsWork_.amplitudes.coeff(nnlsWork_.tsOffset + offset) <= 0.f ) {
      invCovMat.setZero(nnlsWork_.tsSize, nnlsWork_.tsSize);
      continue;
    }
    else {
      invCovMat.noalias() += amp * amp * pulseCovArray[offset + nnlsWork_.bxOffset];
    }
  }
