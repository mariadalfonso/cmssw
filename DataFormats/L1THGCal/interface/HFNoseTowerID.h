#ifndef DataFormats_L1TCalorimeter_HFNoseTowerID_h
#define DataFormats_L1TCalorimeter_HFNoseTowerID_h

// NOTE: in the current implementation HFNoselTowerID can only
// accomodate 127 bins per coordinate x2 zsides

namespace l1t {
  class HFNoseTowerID {
  public:
    HFNoseTowerID() : HFNoseTowerID(0) {}

    HFNoseTowerID(unsigned short rawId) : rawId_(rawId) {}

    HFNoseTowerID(short zside, unsigned short coord1, unsigned short coord2) {
  ...
  struct HFNoseTowerCoord {
    HFNoseTowerCoord(unsigned short rawId, float eta, float phi) : rawId(rawId), eta(eta), phi(phi) {}
