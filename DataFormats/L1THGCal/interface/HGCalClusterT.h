...
class HGCalClusterT : public L1Candidate {
 public:
  typedef typename std::unordered_map<uint32_t, edm::Ptr<C>>::const_iterator
      const_iterator;
 ...
 public:
  ...
  void addConstituent(const edm::Ptr<C>& c, bool updateCentre = true,
                      float fraction = 1.) {
  ...
  double hOverE() const {
    ...
    for (const auto& id_constituent : constituents()) {
      ...
      double fraction =
          (id_fraction != constituentsFraction_.end() ? id_fraction->second
                                                      : 1.);
      ...
      if ((id.det() == DetId::Forward && id.subdetId() == HGCEE) ||
          ...
          //	    (id.det() == DetId::HGCalTrigger &&
          // HFNoseTriggerDetId(id).subdet() ==
          // HGCalTriggerSubdetector::HFNoseTrigger &&
          // HFNoseTriggerDetId(id).isEE())
          (id.det() == DetId::HGCalTrigger &&
           id.subdetId() == HGCalTriggerSubdetector::HFNoseTrigger)) {
 ...
 private:
  ...
  void updateP4AndPosition(const edm::Ptr<C>& c, bool updateCentre = true,
                           float fraction = 1.) {
    double cMipt = c->mipPt() * fraction;
