...
class HGCalTriggerGeometryBase {
 public:
  ...
  const HGCalGeometry* noseGeometry() const {
    return (hgc_nose_geometry_ ? hgc_nose_geometry_
                               : (static_cast<const HGCalGeometry*>(
                                     calo_geometry_->getSubdetectorGeometry(
                                         DetId::Forward, HFNose))));
  }

  const HGCalGeometry* eeGeometry() const {
    return (hgc_ee_geometry_ ? hgc_ee_geometry_
                             : (static_cast<const HGCalGeometry*>(
                                   calo_geometry_->getSubdetectorGeometry(
                                       DetId::Forward, HGCEE))));
  }
  const HGCalGeometry* fhGeometry() const {
    return (hgc_hsi_geometry_ ? hgc_hsi_geometry_
                              : (static_cast<const HGCalGeometry*>(
                                    calo_geometry_->getSubdetectorGeometry(
                                        DetId::Forward, HGCHEF))));
  }
  const HcalGeometry* bhGeometry() const {
    ...
    return (static_cast<const HcalGeometry*>(
        calo_geometry_->getSubdetectorGeometry(DetId::Hcal, HcalEndcap)));
  }
  ...
  virtual void initialize(const HGCalGeometry*, const HGCalGeometry*,
                          const HGCalGeometry*) = 0;
  virtual void initialize(const HGCalGeometry*, const HGCalGeometry*,
                          const HGCalGeometry*, const HGCalGeometry*) = 0;
  virtual void reset();
  ...
  virtual unsigned getModuleFromTriggerCell(
      const unsigned trigger_cell_det_id) const = 0;

  virtual geom_set getCellsFromTriggerCell(
      const unsigned cell_det_id) const = 0;
  virtual geom_set getCellsFromModule(const unsigned cell_det_id) const = 0;
  virtual geom_set getTriggerCellsFromModule(
      const unsigned trigger_cell_det_id) const = 0;

  virtual geom_ordered_set getOrderedCellsFromModule(
      const unsigned cell_det_id) const = 0;
  virtual geom_ordered_set getOrderedTriggerCellsFromModule(
      const unsigned trigger_cell_det_id) const = 0;

  virtual geom_set getNeighborsFromTriggerCell(
      const unsigned trigger_cell_det_id) const = 0;
  ...
  virtual GlobalPoint getTriggerCellPosition(
      const unsigned trigger_cell_det_id) const = 0;
...
typedef edmplugin::PluginFactory<HGCalTriggerGeometryBase*(
