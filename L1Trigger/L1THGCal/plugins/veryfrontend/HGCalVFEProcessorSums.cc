...
void HGCalVFEProcessorSums::run(
    const HGCalDigiCollection& digiColl,
  ...
  for (const auto& digiData : digiColl) {
    if (DetId(digiData.id()).det() == DetId::Hcal &&
        HcalDetId(digiData.id()).subdetId() != HcalEndcap)
      continue;
  ...
  vfeSummationImpl_->triggerCellSums(*geometry_, linearized_dataframes,
                                     payload);
  ...
  for (const auto& id_value : payload) {
    if (id_value.second > 0) {
      l1t::HGCalTriggerCell triggerCell(reco::LeafCandidate::LorentzVector(),
                                        compressed_payload[id_value.first][1],
                                        0, 0, 0, id_value.first);
      ...
      math::PtEtaPhiMLorentzVector p4(
          (double)id_value.second / cosh(point.eta()), point.eta(), point.phi(),
          0.);
