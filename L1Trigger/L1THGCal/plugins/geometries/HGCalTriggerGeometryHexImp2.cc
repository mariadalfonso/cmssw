...
class HGCalTriggerGeometryHexImp2 : public HGCalTriggerGeometryBase {
 ...
 private:
  ...
  std::map<std::pair<short, short>, short>
      cells_to_trigger_cells_;  // FIXME: something else than map<pair,short>?
  std::multimap<std::pair<short, short>, short>
      trigger_cells_to_cells_;  // FIXME: something else than map<pair,short>?
  std::unordered_map<short, short>
      number_trigger_cells_in_wafers_;  // the map key is the wafer type
  std::unordered_map<short, short>
      number_cells_in_wafers_;  // the map key is the wafer type
...
HGCalTriggerGeometryHexImp2::HGCalTriggerGeometryHexImp2(
    ...
    : HGCalTriggerGeometryBase(conf),
      ...
      l1tWaferNeighborsMapping_(
          conf.getParameter<edm::FileInPath>("L1TWaferNeighborsMapping")),
      l1tModulesMapping_(
...
void HGCalTriggerGeometryHexImp2::initialize(
...
void HGCalTriggerGeometryHexImp2::initialize(
  ...
  throw cms::Exception("BadGeometry")
      << "HGCalTriggerGeometryHexImp2 geometry cannot be initialized with the "
         "V9 HGCAL geometry";
}
...
void HGCalTriggerGeometryHexImp2::initialize(
  ...
  throw cms::Exception("BadGeometry")
      << "HGCalTriggerGeometryHexImp2 geometry cannot be initialized with the "
         "V9 HGCAL+Nose geometry";
}

unsigned HGCalTriggerGeometryHexImp2::getTriggerCellFromCell(
  ...
  auto trigger_cell_itr =
      cells_to_trigger_cells_.find(std::make_pair(wafer_type, cell));
  if (trigger_cell_itr == cells_to_trigger_cells_.end()) {
    throw cms::Exception("BadGeometry")
        << "HGCalTriggerGeometry: HGCal  cell " << cell
        << " is not mapped to any trigger cell for the wafer type "
        << wafer_type << ". The trigger cell mapping should be modified.\n";
  }
  unsigned trigger_cell = trigger_cell_itr->second;
  return HGCalDetId((ForwardSubdetector)cell_det_id.subdetId(),
                    cell_det_id.zside(), cell_det_id.layer(),
                    cell_det_id.waferType(), cell_det_id.wafer(), trigger_cell)
      .rawId();
}

unsigned HGCalTriggerGeometryHexImp2::getModuleFromCell(
    const unsigned cell_id) const {
  if (DetId(cell_id).det() == DetId::Hcal) return 0;
  HGCalDetId cell_det_id(cell_id);
  ...
  return HGCalDetId((ForwardSubdetector)cell_det_id.subdetId(),
                    cell_det_id.zside(), cell_det_id.layer(),
                    cell_det_id.waferType(), module, HGCalDetId::kHGCalCellMask)
      .rawId();
}

unsigned HGCalTriggerGeometryHexImp2::getModuleFromTriggerCell(
...
HGCalTriggerGeometryHexImp2::getCellsFromModule(
  ...
  std::pair<std::unordered_multimap<short, short>::const_iterator,
            std::unordered_multimap<short, short>::const_iterator>
      wafer_itrs;
  switch (subdet) {
    case ForwardSubdetector::HGCEE:
  ...
  for (auto wafer_itr = wafer_itrs.first; wafer_itr != wafer_itrs.second;
    ...
    for (int cell = 0; cell < number_cells_in_wafers_.at(wafer_type); cell++) {
      HGCalDetId cell_id((ForwardSubdetector)module_det_id.subdetId(),
                         ...
                         wafer_type, wafer_itr->second, cell);
  ...
  for (auto wafer_itr = wafer_itrs.first; wafer_itr != wafer_itrs.second;
    ...
    for (int trigger_cell = 0;
      ...
      HGCalDetId trigger_cell_id((ForwardSubdetector)module_det_id.subdetId(),
                                 module_det_id.zside(), module_det_id.layer(),
                                 wafer_type, wafer_itr->second, trigger_cell);
...
HGCalTriggerGeometryHexImp2::getNeighborsFromTriggerCell(
  ...
  // the trigger cell)
  std::unordered_map<short, std::vector<short>>::const_iterator
      surrounding_wafers_itr;
  bool out_of_range_error = false;
  switch (subdet) {
    case ForwardSubdetector::HGCEE:
  ...
  types.reserve(surrounding_wafers.size() +
                1);  // includes the central wafer -> +1
  ...
  for (const auto& wafer_tc : neighbors) {
    if (wafer_tc.first - 1 >= (int)surrounding_wafers.size()) {
      throw cms::Exception("BadGeometry")
          << "HGCalTriggerGeometry: Undefined wafer neighbor number "
          << wafer_tc.first << " for wafer " << wafer << " and trigger cell "
          << trigger_cell << ". The neighbor mapping files should be modified.";
    }
    int neighbor_wafer =
        (wafer_tc.first == 0 ? wafer
                             : surrounding_wafers.at(wafer_tc.first - 1));
    ...
    HGCalDetId neighbor_det_id(
        ...
        trigger_cell_det_id.zside(), trigger_cell_det_id.layer(), type,
        neighbor_wafer, wafer_tc.second);
...
unsigned HGCalTriggerGeometryHexImp2::getLinksInModule(
    const unsigned module_id) const {
  return 1;
}

unsigned HGCalTriggerGeometryHexImp2::getModuleSize(
    const unsigned module_id) const {
  return 1;
}

GlobalPoint HGCalTriggerGeometryHexImp2::getTriggerCellPosition(
...
GlobalPoint HGCalTriggerGeometryHexImp2::getModulePosition(
...
void HGCalTriggerGeometryHexImp2::fillMaps() {
  ...
  for (; l1tModulesMappingStream >> subdet >> wafer >> module;) {
    ...
    switch (subdet) {
      case ForwardSubdetector::HGCEE: {
        ...
        number_cells_in_wafers_.emplace(
            wafer_type, eeTopology().dddConstants().numberCellsHexagon(wafer));
        break;
      }
      case ForwardSubdetector::HGCHEF: {
        // fill module <-> wafers mappings
        ...
        // fill number of cells for a given wafer type
        number_cells_in_wafers_.emplace(
            wafer_type, fhTopology().dddConstants().numberCellsHexagon(wafer));
        break;
      }
      default:
  ...
  if (!l1tModulesMappingStream.eof())
    edm::LogWarning("HGCalTriggerGeometry")
        << "Error reading L1TModulesMapping '" << wafer << " " << module
        << "' \n";
  ...
  for (; l1tCellsMappingStream >> waferType >> cell >> triggerCell;) {
    ...
    cells_to_trigger_cells_.emplace(std::make_pair((waferType ? 1 : -1), cell),
                                    triggerCell);
    trigger_cells_to_cells_.emplace(
        std::make_pair((waferType ? 1 : -1), triggerCell), cell);
    // fill number of cells for a given wafer type
    auto itr_insert =
        number_trigger_cells_in_wafers_.emplace((waferType ? 1 : -1), 0);
    if (triggerCell + 1 > itr_insert.first->second)
      itr_insert.first->second = triggerCell + 1;
  }
  if (!l1tCellsMappingStream.eof())
    edm::LogWarning("HGCalTriggerGeometry")
        << "Error reading L1TCellsMapping'" << waferType << " " << cell << " "
        << triggerCell << "' \n";
...
void HGCalTriggerGeometryHexImp2::fillNeighborMaps() {
  ...
  std::ifstream l1tCellNeighborsMappingStream(
      l1tCellNeighborsMapping_.fullPath());
  ...
  for (std::array<char, 512> buffer;
    ...
    auto itr_insert = trigger_cell_neighbors_.emplace(
        map_key, std::set<std::pair<short, short>>());
  ...
  std::ifstream l1tWaferNeighborsMappingStream(
      l1tWaferNeighborsMapping_.fullPath());
  if (!l1tWaferNeighborsMappingStream.is_open()) {
    throw cms::Exception("MissingDataFile")
        << "Cannot open HGCalTriggerGeometry L1TWaferNeighborsMapping file\n";
  }
  for (std::array<char, 512> buffer;
       l1tWaferNeighborsMappingStream.getline(&buffer[0], 512);) {
    std::string line(&buffer[0]);
    ...
    if (tokens.size() != 8) {
      throw cms::Exception("BadGeometry")
          ...
          << "  subdet waferid neighbor1 neighbor2 neighbor3 neighbor4 "
             "neighbor5 neighbor6\n";
    }
    ...
    for (auto neighbor_itr = tokens.cbegin() + 2; neighbor_itr != tokens.cend();
         ++neighbor_itr) {
  ...
  for (unsigned layer = 1; layer <= n_layers_ee; layer++) {
    for (const auto& wafer_module : wafer_to_module_ee_) {
      ...
      for (int trigger_cell = 0;
           trigger_cell < number_trigger_cells_in_wafers_.at(wafer_type);
           trigger_cell++) {
        HGCalDetId trigger_cell_id_neg(ForwardSubdetector::HGCEE, -1, layer,
                                       wafer_type, wafer, trigger_cell);
        HGCalDetId trigger_cell_id_pos(ForwardSubdetector::HGCEE, 1, layer,
                                       wafer_type, wafer, trigger_cell);
  ...
  for (unsigned layer = 1; layer <= n_layers_fh; layer++) {
    for (const auto& wafer_module : wafer_to_module_fh_) {
      ...
      for (int trigger_cell = 0;
           trigger_cell < number_trigger_cells_in_wafers_.at(wafer_type);
           trigger_cell++) {
        HGCalDetId trigger_cell_id_neg(ForwardSubdetector::HGCHEF, -1, layer,
                                       wafer_type, wafer, trigger_cell);
        HGCalDetId trigger_cell_id_pos(ForwardSubdetector::HGCHEF, 1, layer,
                                       wafer_type, wafer, trigger_cell);
        if (!validTriggerCellFromCells(trigger_cell_id_neg))
          invalid_triggercells_.emplace(trigger_cell_id_neg.rawId());
        if (!validTriggerCellFromCells(trigger_cell_id_pos))
...
unsigned HGCalTriggerGeometryHexImp2::packTriggerCell(
    unsigned trigger_cell, const std::vector<int>& wafer_types) const {
...
bool HGCalTriggerGeometryHexImp2::validTriggerCell(
    const unsigned trigger_cell_id) const {
  return invalid_triggercells_.find(trigger_cell_id) ==
         invalid_triggercells_.end();
}

bool HGCalTriggerGeometryHexImp2::disconnectedModule(
    const unsigned module_id) const {
  return false;
}

unsigned HGCalTriggerGeometryHexImp2::lastTriggerLayer() const {
...
bool HGCalTriggerGeometryHexImp2::validTriggerCellFromCells(
    const unsigned trigger_cell_id) const {
