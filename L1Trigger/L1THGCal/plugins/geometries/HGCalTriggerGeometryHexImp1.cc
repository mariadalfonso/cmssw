...
/*****************************************************************/
{
  ...
  for (; l1tModulesMappingStream >> subdet >> wafer >> module;) {
    if (subdet == 3) {
      ...
      auto itr_insert =
          module_to_wafers_ee.emplace(module, std::map<short, short>());
      itr_insert.first->second.emplace(wafer, 0);
    } else if (subdet == 4) {
      wafer_to_module_fh.emplace(wafer, module);
      auto itr_insert =
          module_to_wafers_fh.emplace(module, std::map<short, short>());
      itr_insert.first->second.emplace(wafer, 0);
    } else
      edm::LogWarning("HGCalTriggerGeometry")
  ...
  std::unordered_map<short, short>
      number_trigger_cells_in_wafers;  // the map key is the wafer type
  ...
  for (; l1tCellsMappingStream >> waferType >> cell >> triggerCell;) {
    cells_to_trigger_cells.emplace(std::make_pair((waferType ? 1 : -1), cell),
                                   triggerCell);
    auto itr_insert =
        number_trigger_cells_in_wafers.emplace((waferType ? 1 : -1), 0);
  ...
  // For each wafer compute the trigger cell offset according to the wafer
  // position inside the module. The first wafer will have an offset equal to 0,
  // the second an offset equal to the number of trigger cells in the first
  // wafer, etc.
  short offset = 0;
  for (auto& module_wafers : module_to_wafers_ee) {
    ...
    for (auto& wafer_offset : module_wafers.second) {
      ...
      int wafer_type =
          (eeTopology().dddConstants().waferTypeT(wafer_offset.first) == 1
               ? 1
               : -1);
  ...
  for (auto& module_wafers : module_to_wafers_fh) {
    ...
    for (auto& wafer_offset : module_wafers.second) {
      wafer_offset.second = offset;
      int wafer_type =
          (fhTopology().dddConstants().waferTypeT(wafer_offset.first) == 1
               ? 1
               : -1);
      offset += number_trigger_cells_in_wafers.at(wafer_type);
    }
  ...
  for (const auto& id : eeGeometry()->getValidGeomDetIds()) {
    ...
    short triggercell_offset =
        module_to_wafers_ee.at(module).at(waferDetId.wafer());
    int nCells =
        eeTopology().dddConstants().numberCellsHexagon(waferDetId.wafer());
    for (int c = 0; c < nCells; c++) {
      short triggerCellId =
          cells_to_trigger_cells[std::make_pair(waferDetId.waferType(), c)];
      // Fill cell -> trigger cell mapping
      HGCalDetId cellDetId(ForwardSubdetector(waferDetId.subdetId()),
                           waferDetId.zside(), waferDetId.layer(),
                           waferDetId.waferType(), waferDetId.wafer(), c);
      // The module id is used instead of the wafer id for the trigger cells
      // Since there are several wafers per module, an offset is applied on the
      // HGCalDetId::cell field
      if (triggercell_offset + triggerCellId >= HGCalDetId::kHGCalCellMask)
        edm::LogError("HGCalTriggerGeometry")
            << "Trigger cell id requested with a cell field larger than "
               "available in HGCalDetId ("
  ...
  for (const auto& id : fhGeometry()->getValidGeomDetIds()) {
    ...
    short triggercell_offset =
        module_to_wafers_fh.at(module).at(waferDetId.wafer());
    int nCells =
        fhTopology().dddConstants().numberCellsHexagon(waferDetId.wafer());
    for (int c = 0; c < nCells; c++) {
      short triggerCellId =
          cells_to_trigger_cells[std::make_pair(waferDetId.waferType(), c)];
      // Fill cell -> trigger cell mapping
...
/*****************************************************************/
{
  ...
  for (const auto& triggerCell_cells : trigger_cells_to_cells) {
    ...
    const auto& triggerCellToModuleItr =
        trigger_cells_to_modules_.find(triggerCellId);
    unsigned moduleId =
        (triggerCellToModuleItr != trigger_cells_to_modules_.end()
  ...
  for (const auto& triggerCell_module : trigger_cells_to_modules_) {
    ...
    auto itr_exist =
        modules_to_trigger_cells.emplace(module, list_triggerCells());
  ...
  for (const auto& module_triggerCell : modules_to_trigger_cells) {
    ...
    modules_.emplace(moduleId,
                     std::make_unique<const HGCalTriggerGeometry::Module>(
                         moduleId, modulePoint, list_triggerCells(),
                         triggerCellIds, cellsInTriggerCells));
  }
}

DEFINE_EDM_PLUGIN(HGCalTriggerGeometryFactory, HGCalTriggerGeometryHexImp1,
