...
/*****************************************************************/
{
  throw cms::Exception("BadGeometry")
      << "HGCalTriggerGeometryImp1 geometry cannot be initialized with the V9 "
         "HGCAL geometry";
}

/*****************************************************************/
void HGCalTriggerGeometryImp1::initialize(
...
/*****************************************************************/
{
  ...
  short module = 0;
  short triggercell = 0;
  for (; l1tCellsMappingStream >> layer >> subsector >> cell >> module >>
         triggercell;) {
    if (layer > 30 || layer <= 0) {
      edm::LogWarning("HGCalTriggerGeometry")
    ...
    for (unsigned z = 0; z <= 1; z++) {
      ...
      for (unsigned sector = 1; sector <= 18; sector++) {
        ...
        HGCTriggerDetId triggerDetid(HGCTrigger, zside, layer, sector, module,
                                     triggercell);
        const auto& ret =
            cells_to_trigger_cells_.insert(std::make_pair(detid, triggerDetid));
        if (!ret.second)
          edm::LogWarning("HGCalTriggerGeometry")
              << "Duplicate cell in L1TCellsMapping\n";
        // Fill trigger cell -> module mapping
        HGCTriggerDetId moduleDetid(HGCTrigger, zside, layer, sector, module,
                                    HGCTriggerDetId::UndefinedCell());
        trigger_cells_to_modules_.insert(std::make_pair(
            triggerDetid, moduleDetid));  // do nothing if trigger cell has
  ...
  if (!l1tCellsMappingStream.eof())
    edm::LogWarning("HGCalTriggerGeometry")
        << "Error reading L1TCellsMapping'" << layer << " " << cell << " "
        << triggercell << " " << subsector << "' \n";
  ...
  for (const auto& triggercell_cells : trigger_cells_to_cells) {
    ...
    unsigned moduleId =
        (tc2mItr != trigger_cells_to_modules_.end()
             ? tc2mItr->second
    ...
    std::unique_ptr<const HGCalTriggerGeometry::TriggerCell> triggercellPtr(
        new HGCalTriggerGeometry::TriggerCell(
            triggercellId, moduleId, triggercellPoint, list_cells(), cellIds));
    trigger_cells_.insert(
        std::make_pair(triggercellId, std::move(triggercellPtr)));
  }
  //
  ...
  for (const auto& triggercell_module : trigger_cells_to_modules_) {
    ...
    modules_to_trigger_cells.insert(
        std::make_pair(module, list_triggercells()));
  ...
  for (const auto& module_triggercell : modules_to_trigger_cells) {
    ...
    // Position: for the moment, barycenter of the module, from trigger cell
    // positions
    ...
    // FIXME: empty neighbours
    std::unique_ptr<const HGCalTriggerGeometry::Module> modulePtr(
        new HGCalTriggerGeometry::Module(moduleId, modulePoint,
                                         list_triggercells(), triggercellIds,
                                         cellsInTriggerCells));
    modules_.insert(std::make_pair(moduleId, std::move(modulePtr)));
  }
}

DEFINE_EDM_PLUGIN(HGCalTriggerGeometryFactory, HGCalTriggerGeometryImp1,
