...
class HGCalTriggerGeometryHexLayerBasedImp1 : public HGCalTriggerGeometryBase {
 ...
 private:
  ...
  std::unordered_map<int, std::set<std::pair<short, short>>>
...
HGCalTriggerGeometryHexLayerBasedImp1::HGCalTriggerGeometryHexLayerBasedImp1(
    ...
    : HGCalTriggerGeometryBase(conf),
      ...
      l1tCellsBHMapping_(
          conf.getParameter<edm::FileInPath>("L1TCellsBHMapping")),
      l1tModulesMapping_(
      ...
      l1tCellNeighborsBHMapping_(
          conf.getParameter<edm::FileInPath>("L1TCellNeighborsBHMapping")) {
...
void HGCalTriggerGeometryHexLayerBasedImp1::initialize(
    const CaloGeometry* calo_geometry) {
...
void HGCalTriggerGeometryHexLayerBasedImp1::initialize(
  ...
  throw cms::Exception("BadGeometry")
      << "HGCalTriggerGeometryHexLayerBasedImp1 geometry cannot be initialized "
         "with the V9 HGCAL geometry";
}
...
void HGCalTriggerGeometryHexLayerBasedImp1::initialize(
  ...
  throw cms::Exception("BadGeometry")
      << "HGCalTriggerGeometryHexLayerBasedImp1 geometry cannot be initialized "
         "with the V9 HGCAL+Nose geometry";
}

unsigned HGCalTriggerGeometryHexLayerBasedImp1::getTriggerCellFromCell(
  ...
  if (DetId(cell_id).det() == DetId::Hcal) {
    ...
    auto trigger_cell_itr =
        cells_to_trigger_cells_bh_.find(packIetaIphi(ieta, iphi));
  ...
  // EE or FH
    ...
    auto trigger_cell_itr =
        cells_to_trigger_cells_.find(packWaferCellId(subdet, wafer, cell));
    if (trigger_cell_itr == cells_to_trigger_cells_.end()) {
      throw cms::Exception("BadGeometry")
          << "HGCalTriggerGeometry: HGCal  cell " << cell << " in wafer "
          << wafer
          << " is not mapped to any trigger cell. The trigger cell mapping "
             "should be modified.\n";
    }
  ...
  return HGCalDetId((ForwardSubdetector)subdet, zside, layer, 1,
                    wafer_trigger_cell, trigger_cell)
      .rawId();
}

unsigned HGCalTriggerGeometryHexLayerBasedImp1::getModuleFromCell(
    const unsigned cell_id) const {
  return getModuleFromTriggerCell(getTriggerCellFromCell(cell_id));
}

unsigned HGCalTriggerGeometryHexLayerBasedImp1::getModuleFromTriggerCell(
  ...
  return HGCalDetId((ForwardSubdetector)trigger_cell_det_id.subdetId(),
                    ...
                    (trigger_cell_det_id.waferType() == 1 ? 1 : 0), module,
...
HGCalTriggerGeometryHexLayerBasedImp1::getCellsFromTriggerCell(
  ...
  if (trigger_cell_det_id.subdetId() == ForwardSubdetector::HGCHEB) {
    ...
    const auto& cell_range = trigger_cells_to_cells_bh_.equal_range(
    ...
    for (auto tc_c_itr = cell_range.first; tc_c_itr != cell_range.second;
      ...
      unsigned cell_det_id =
          HcalDetId(HcalEndcap, trigger_cell_det_id.zside() * ieta, iphi,
  ...
  // EE or FH
    ...
    for (auto tc_c_itr = cell_range.first; tc_c_itr != cell_range.second;
      ...
      unsigned cell_det_id =
          HGCalDetId((ForwardSubdetector)trigger_cell_det_id.subdetId(),
                     trigger_cell_det_id.zside(), trigger_cell_det_id.layer(),
                     wafer_type, wafer, cell)
              .rawId();
      if (validCellId(subdet, cell_det_id)) cell_det_ids.emplace(cell_det_id);
    }
  }
...
HGCalTriggerGeometryHexLayerBasedImp1::getTriggerCellsFromModule(
  ...
  if (module_det_id.subdetId() == ForwardSubdetector::HGCHEB) {
    ...
    for (int trigger_cell = 0;
         trigger_cell < number_trigger_cells_in_wafers_bh_.at(module);
         trigger_cell++) {
      HGCalDetId trigger_cell_id((ForwardSubdetector)module_det_id.subdetId(),
                                 module_det_id.zside(), module_det_id.layer(),
                                 1, module, trigger_cell);
  ...
  // EE or FH
    ...
    for (auto wafer_itr = wafer_itrs.first; wafer_itr != wafer_itrs.second;
      ...
      for (int trigger_cell = 0;
           trigger_cell < number_trigger_cells_in_wafers_.at(wafer);
           trigger_cell++) {
        HGCalDetId trigger_cell_id((ForwardSubdetector)module_det_id.subdetId(),
                                   module_det_id.zside(), module_det_id.layer(),
                                   1, wafer, trigger_cell);
        if (validTriggerCell(trigger_cell_id))
          trigger_cell_det_ids.emplace(trigger_cell_id.rawId());
      }
    }
...
HGCalTriggerGeometryHexLayerBasedImp1::getNeighborsFromTriggerCell(
  ...
  for (const auto& module_tc : neighbors) {
    HGCalDetId neighbor_det_id(
        ...
        trigger_cell_det_id.zside(), trigger_cell_det_id.layer(), 1,
        module_tc.first, module_tc.second);
...
unsigned HGCalTriggerGeometryHexLayerBasedImp1::getLinksInModule(
    const unsigned module_id) const {
  return 1;
}

unsigned HGCalTriggerGeometryHexLayerBasedImp1::getModuleSize(
    const unsigned module_id) const {
  return 1;
}

GlobalPoint HGCalTriggerGeometryHexLayerBasedImp1::getTriggerCellPosition(
  ...
  // EE or FH
    ...
    for (const auto& cell : cell_ids) {
      ...
      triggerCellVector += (cellDetId.subdetId() == ForwardSubdetector::HGCEE
...
GlobalPoint HGCalTriggerGeometryHexLayerBasedImp1::getModulePosition(
  ...
  // EE or FH
    ...
    for (const auto& cell : cell_ids) {
      HGCalDetId cellDetId(cell);
      moduleVector += (cellDetId.subdetId() == ForwardSubdetector::HGCEE
                           ? eeGeometry()->getPosition(cellDetId)
                           : fhGeometry()->getPosition(cellDetId))
                          .basicVector();
    }
...
void HGCalTriggerGeometryHexLayerBasedImp1::fillMaps() {
  ...
  for (; l1tCellsMappingStream >> subdet >> wafer >> cell >> trigger_wafer >>
         trigger_cell;) {
  ...
  if (!l1tCellsMappingStream.eof())
    edm::LogWarning("HGCalTriggerGeometry")
        << "Error reading L1TCellsMapping '" << subdet << " " << wafer << " "
  ...
  trigger_wafer = 0;
  trigger_cell = 0;
  for (; l1tCellsBHMappingStream >> ieta >> iphi >> trigger_wafer >>
         trigger_cell;) {
    ...
    // fill number of trigger cells in wafers
    auto itr_insert =
        number_trigger_cells_in_wafers_bh_.emplace(trigger_wafer, 0);
    if (trigger_cell + 1 > itr_insert.first->second)
      itr_insert.first->second = trigger_cell + 1;
  }
  if (!l1tCellsBHMappingStream.eof())
    edm::LogWarning("HGCalTriggerGeometry")
        << "Error reading L1TCellsBHMapping '" << ieta << " " << iphi << " "
        << trigger_wafer << " " << trigger_cell << "' \n";
  l1tCellsBHMappingStream.close();
}

void HGCalTriggerGeometryHexLayerBasedImp1::fillNeighborMaps(
    const edm::FileInPath& file,
  ...
  for (std::array<char, 512> buffer;
       l1tCellNeighborsMappingStream.getline(&buffer[0], 512);) {
  ...
  for (unsigned layer = 1; layer <= n_layers_ee; layer++) {
    for (const auto& wafer_module : wafer_to_module_) {
      ...
      for (int trigger_cell = 0;
        ...
        trigger_cell_ids.emplace(HGCalDetId(ForwardSubdetector::HGCEE, -1,
                                            layer, 1, wafer, trigger_cell));
        trigger_cell_ids.emplace(HGCalDetId(ForwardSubdetector::HGCEE, 1, layer,
                                            1, wafer, trigger_cell));
        trigger_cell_ids.emplace(HGCalDetId(ForwardSubdetector::HGCHEF, -1,
                                            layer, 1, wafer, trigger_cell));
        trigger_cell_ids.emplace(HGCalDetId(ForwardSubdetector::HGCHEF, 1,
                                            layer, 1, wafer, trigger_cell));
...
unsigned HGCalTriggerGeometryHexLayerBasedImp1::packWaferCellId(
    unsigned subdet, unsigned wafer, unsigned cell) const {
...
unsigned HGCalTriggerGeometryHexLayerBasedImp1::packIetaIphi(
    unsigned ieta, unsigned iphi) const {
  unsigned packed_value = 0;
  packed_value |= (iphi & HcalDetId::kHcalPhiMask2);
  packed_value |=
      ((ieta & HcalDetId::kHcalEtaMask2) << HcalDetId::kHcalEtaOffset2);
  return packed_value;
}

void HGCalTriggerGeometryHexLayerBasedImp1::unpackWaferCellId(
    unsigned wafer_cell, unsigned& wafer, unsigned& cell) const {
  cell = wafer_cell & HGCalDetId::kHGCalCellMask;
  wafer = (wafer_cell >> HGCalDetId::kHGCalWaferOffset) &
          HGCalDetId::kHGCalWaferMask;
}

void HGCalTriggerGeometryHexLayerBasedImp1::unpackIetaIphi(
    unsigned ieta_iphi, unsigned& ieta, unsigned& iphi) const {
  iphi = ieta_iphi & HcalDetId::kHcalPhiMask2;
  ieta = (ieta_iphi >> HcalDetId::kHcalEtaOffset2) & HcalDetId::kHcalEtaMask2;
}

bool HGCalTriggerGeometryHexLayerBasedImp1::validTriggerCell(
    const unsigned trigger_cell_id) const {
  return invalid_triggercells_.find(trigger_cell_id) ==
         invalid_triggercells_.end();
}

bool HGCalTriggerGeometryHexLayerBasedImp1::disconnectedModule(
    const unsigned module_id) const {
...
unsigned HGCalTriggerGeometryHexLayerBasedImp1::triggerLayer(
...
bool HGCalTriggerGeometryHexLayerBasedImp1::validTriggerCellFromCells(
    const unsigned trigger_cell_id) const {
...
bool HGCalTriggerGeometryHexLayerBasedImp1::validCellId(
    unsigned subdet, unsigned cell_id) const {
  bool is_valid = false;
  switch (subdet) {
...
unsigned HGCalTriggerGeometryHexLayerBasedImp1::packTriggerCell(
...
int HGCalTriggerGeometryHexLayerBasedImp1::detIdWaferType(unsigned subdet,
  ...
  switch (subdet) {
    ...
    case ForwardSubdetector::HGCEE:
      wafer_type =
          (eeTopology().dddConstants().waferTypeT(wafer) == 2 ? -1 : 1);
      break;
    case ForwardSubdetector::HGCHEF:
      wafer_type =
          (fhTopology().dddConstants().waferTypeT(wafer) == 2 ? -1 : 1);
      break;
    default:
      break;
  };
...
unsigned HGCalTriggerGeometryHexLayerBasedImp1::layerWithOffset(
    unsigned id) const {
  HGCalDetId detid(id);
  unsigned layer = 0;
