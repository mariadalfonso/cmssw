...
HGCalTriggerGeometryV9Imp2::HGCalTriggerGeometryV9Imp2(
    ...
    : HGCalTriggerGeometryBase(conf),
      hSc_triggercell_size_(
          conf.getParameter<unsigned>("ScintillatorTriggerCellSize")),
...
void HGCalTriggerGeometryV9Imp2::initialize(const CaloGeometry* calo_geometry) {
  throw cms::Exception("BadGeometry")
      << "HGCalTriggerGeometryV9Imp2 geometry cannot be initialized with the "
         "V7/V8 HGCAL geometry";
}

void HGCalTriggerGeometryV9Imp2::initialize(
...
unsigned HGCalTriggerGeometryV9Imp2::getTriggerCellFromCell(
  ...
  if (det == DetId::HGCalHSc) {
    ...
    int ieta = ((cell_sc_id.ietaAbs() - 1) / hSc_triggercell_size_ + 1) *
               cell_sc_id.zside();
    int iphi = (cell_sc_id.iphi() - 1) / hSc_triggercell_size_ + 1;
    trigger_cell_id =
        HGCScintillatorDetId(cell_sc_id.type(), cell_sc_id.layer(), ieta, iphi)
            .rawId();
  }
  // HFNose
  else if (det == DetId::Forward &&
           DetId(cell_id).subdetId() == ForwardSubdetector::HFNose) {
    HFNoseDetId cell_nose_id(cell_id);
    trigger_cell_id =
        HFNoseTriggerDetId(HGCalTriggerSubdetector::HFNoseTrigger,
                           cell_nose_id.zside(), cell_nose_id.type(),
                           cell_nose_id.layer(), cell_nose_id.waferU(),
                           cell_nose_id.waferV(), cell_nose_id.triggerCellU(),
  ...
  // Silicon
    ...
    trigger_cell_id =
        HGCalTriggerDetId(
            det == DetId::HGCalEE ? HGCalTriggerSubdetector::HGCalEETrigger
                                  : HGCalTriggerSubdetector::HGCalHSiTrigger,
            cell_si_id.zside(), cell_si_id.type(), cell_si_id.layer(),
            cell_si_id.waferU(), cell_si_id.waferV(), cell_si_id.triggerCellU(),
...
unsigned HGCalTriggerGeometryV9Imp2::getModuleFromCell(
    const unsigned cell_id) const {
  return getModuleFromTriggerCell(getTriggerCellFromCell(cell_id));
}

unsigned HGCalTriggerGeometryV9Imp2::getModuleFromTriggerCell(
  ...
  if (det == DetId::HGCalHSc) {
    ...
    int ieta =
        ((trigger_cell_sc_id.ietaAbs() - ietamin_tc) / hSc_module_size_ + 1) *
        zside;
  ...
  // HFNose
           ...
           HGCalTriggerDetId(trigger_cell_id).subdet() ==
    ...
    HFNoseDetId module = hfn.getModule(HFNoseDetId(zside, layer, tc_type, waferu, waferv, 0, 0));
    module_id = module.rawId();
  ...
  // Silicon
    ...
    subdet_old = (subdet == HGCalTriggerSubdetector::HGCalEETrigger
                      ? ForwardSubdetector::HGCEE
    ...
    if (subdet == HGCalTriggerSubdetector::HGCalEETrigger ||
      ...
      if (module_itr == wafer_to_module_.end()) {
        throw cms::Exception("BadGeometry")
            << trigger_cell_trig_id << "HGCalTriggerGeometry: Wafer (" << waferu
            << "," << waferv
            << ") is not mapped to any trigger module. The module mapping "
               "should be modified. \n";
      }
      module = module_itr->second;
    }
    module_id = HGCalDetId((ForwardSubdetector)subdet_old, zside, layer,
                           tc_type, module, HGCalDetId::kHGCalCellMask)
                    .rawId();
  }
  return module_id;
}
...
HGCalTriggerGeometryV9Imp2::getCellsFromTriggerCell(
  ...
  if (det == DetId::HGCalHSc) {
    ...
    for (int ietaAbs = ieta0; ietaAbs < ieta0 + (int)hSc_triggercell_size_;
         ietaAbs++) {
      int ieta = ietaAbs * trigger_cell_sc_id.zside();
      for (int iphi = iphi0; iphi < iphi0 + (int)hSc_triggercell_size_;
           iphi++) {
        unsigned cell_id = HGCScintillatorDetId(
            trigger_cell_sc_id.type(), trigger_cell_sc_id.layer(), ieta, iphi);
  ...
  // HFNose
    ...
    for (unsigned ic = 0; ic < cellus.size(); ic++) {
      HFNoseDetId cell_det_id(zside, type, layer, waferu, waferv, cellus[ic],
                              cellvs[ic]);
  ...
  // Silicon
    ...
    if (subdet == HGCalTriggerSubdetector::HGCalEETrigger ||
      ...
      DetId::Detector cell_det =
          (subdet == HGCalTriggerSubdetector::HGCalEETrigger ? DetId::HGCalEE
                                                             : DetId::HGCalHSi);
      ...
      for (unsigned ic = 0; ic < cellus.size(); ic++) {
        HGCSiliconDetId cell_det_id(cell_det, zside, type, layer, waferu,
                                    waferv, cellus[ic], cellvs[ic]);
        cell_det_ids.emplace(cell_det_id.rawId());
      }
    }
  }
...
HGCalTriggerGeometryBase::geom_set
HGCalTriggerGeometryV9Imp2::getCellsFromModule(const unsigned module_id) const {
  geom_set cell_det_ids;
...
HGCalTriggerGeometryV9Imp2::getTriggerCellsFromModule(
  ...
  if (det == DetId::HGCalHSc) {
    ...
    int ietamin =
        hscTopology().dddConstants().getREtaRange(module_sc_id.layer()).first;
    int ietamin_tc = ((ietamin - 1) / hSc_triggercell_size_ + 1);
    int ieta0 = (module_sc_id.ietaAbs() - 1) * hSc_module_size_ + ietamin_tc;
    int iphi0 = (module_sc_id.iphi() - 1) * hSc_module_size_ + 1;
    for (int ietaAbs = ieta0; ietaAbs < ieta0 + (int)hSc_module_size_;
         ietaAbs++) {
      int ieta = ietaAbs * module_sc_id.zside();
      for (int iphi = iphi0; iphi < iphi0 + (int)hSc_module_size_; iphi++) {
        unsigned trigger_cell_id = HGCScintillatorDetId(
            module_sc_id.type(), module_sc_id.layer(), ieta, iphi);
  ...
  // HFNose
           ...
           module_det_id.subdetId() == ForwardSubdetector::HFNose) {
    HFNoseDetId module_nose_id(module_id);
    HFNoseDetIdToModule hfn;
    std::vector<HFNoseTriggerDetId> ids = hfn.getTriggerDetIds(module_nose_id);
    for (unsigned int tdi=0; tdi < ids.size(); tdi++ ) {
      if (validTriggerCellFromCells(ids[tdi]))
	trigger_cell_det_ids.emplace(ids[tdi].rawId());
    }
  }
  // Silicon
  else {
    ...
    auto wafer_itrs = module_to_wafers_.equal_range(
        packLayerModuleId(layerWithOffset(module_id), module));
    ...
    for (auto wafer_itr = wafer_itrs.first; wafer_itr != wafer_itrs.second;
      ...
      DetId::Detector det =
          (module_si_id.subdetId() == ForwardSubdetector::HGCEE
               ? DetId::HGCalEE
               : DetId::HGCalHSi);
      ...
      for (int roc = 1; roc <= nroc; roc++) {
        ...
        for (const auto& tc_uv : tc_uvs) {
          HGCalTriggerDetId trigger_cell_id(subdet, module_si_id.zside(),
                                            wafer_type, layer, waferu, waferv,
                                            tc_uv.first, tc_uv.second);
          if (validTriggerCellFromCells(trigger_cell_id))
...
HGCalTriggerGeometryV9Imp2::getOrderedTriggerCellsFromModule(
  ...
  // HFNose
           ...
           DetId(module_det_id).subdetId() == ForwardSubdetector::HFNose) {
    HFNoseDetId module_nose_id(module_id);
    HFNoseDetIdToModule hfn;
    std::vector<HFNoseTriggerDetId> ids = hfn.getTriggerDetIds(module_nose_id);
    for (unsigned int tdi=0; tdi < ids.size(); tdi++ ) {
...
HGCalTriggerGeometryV9Imp2::getNeighborsFromTriggerCell(
    const unsigned trigger_cell_id) const {
  throw cms::Exception("FeatureNotImplemented")
      << "Neighbor search is not implemented in HGCalTriggerGeometryV9Imp2";
}

unsigned HGCalTriggerGeometryV9Imp2::getLinksInModule(
    const unsigned module_id) const {
  DetId module_det_id(module_id);
  unsigned links = 0;
  // Scintillator
  ...
  if (module_det_id.det() == DetId::Forward &&
      module_det_id.subdetId() == ForwardSubdetector::HFNose) {
    links = 1;
  }
...
unsigned HGCalTriggerGeometryV9Imp2::getModuleSize(
    const unsigned module_id) const {
  DetId module_det_id(module_id);
...
GlobalPoint HGCalTriggerGeometryV9Imp2::getTriggerCellPosition(
  ...
  // HFNose
           ...
           HGCalTriggerDetId(trigger_cell_det_id).subdet() ==
               HGCalTriggerSubdetector::HFNoseTrigger) {
    for (const auto& cell : cell_ids) {
  ...
  return GlobalPoint(triggerCellVector / static_cast<int>(cell_ids.size()));
}

GlobalPoint HGCalTriggerGeometryV9Imp2::getModulePosition(
  ...
  return GlobalPoint(moduleVector / static_cast<int>(cell_ids.size()));
}
...
void HGCalTriggerGeometryV9Imp2::fillMaps() {
  ...
  for (; l1tModulesMappingStream >> layer >> waferu >> waferv >> module;) {
    ...
    module_to_wafers_.emplace(packLayerModuleId(layer, module),
                              packWaferId(waferu, waferv));
  }
  if (!l1tModulesMappingStream.eof()) {
    throw cms::Exception("BadGeometryFile")
        << "Error reading L1TModulesMapping '" << layer << " " << waferu << " "
        << waferv << " " << module << "' \n";
  }
  ...
  for (; l1tLinksMappingStream >> layer >> module >> links;) {
    if (module_to_wafers_.find(packLayerModuleId(layer, module)) ==
...
unsigned HGCalTriggerGeometryV9Imp2::packWaferId(int waferU, int waferV) const {
  ...
  packed_value |= ((std::abs(waferU) & HGCSiliconDetId::kHGCalWaferUMask)
  ...
  packed_value |= ((std::abs(waferV) & HGCSiliconDetId::kHGCalWaferVMask)
...
unsigned HGCalTriggerGeometryV9Imp2::packLayerWaferId(unsigned layer,
  ...
  packed_value |= ((waferVsign & HGCSiliconDetId::kHGCalWaferVSignMask)
                   << HGCSiliconDetId::kHGCalWaferVSignOffset);
  packed_value |= ((layer & HGCSiliconDetId::kHGCalLayerMask)
...
void HGCalTriggerGeometryV9Imp2::unpackWaferId(unsigned wafer, int& waferU,
                                               int& waferV) const {
...
bool HGCalTriggerGeometryV9Imp2::validTriggerCell(
    const unsigned trigger_cell_id) const {
  return validTriggerCellFromCells(trigger_cell_id);
}

bool HGCalTriggerGeometryV9Imp2::disconnectedModule(
    const unsigned module_id) const {
...
bool HGCalTriggerGeometryV9Imp2::validTriggerCellFromCells(
    const unsigned trigger_cell_id) const {
...
bool HGCalTriggerGeometryV9Imp2::validCellId(unsigned subdet,
...
int HGCalTriggerGeometryV9Imp2::detIdWaferType(unsigned det, unsigned layer,
                                               short waferU,
                                               short waferV) const {
  int wafer_type = 0;
  switch (det) {
...
DEFINE_EDM_PLUGIN(HGCalTriggerGeometryFactory, HGCalTriggerGeometryV9Imp2,
