 ...
 public:
  ...
  void initialize(const CaloGeometry*) final {
    ...
    for (unsigned i = 0; i < nmodules; ++i) {
      ...
      trigger_cells_[i] = std::make_unique<HGCalTriggerGeometry::TriggerCell>(
          i, i, GlobalPoint(), tc_empty, tc_empty);
      ...
      modules_[i] = std::make_unique<HGCalTriggerGeometry::Module>(
          i, GlobalPoint(), mod_empty, mod_comps, map_empty);
    }
  }
...
DEFINE_EDM_PLUGIN(HGCalTriggerGeometryFactory, TrivialGeometry,
                  "TrivialGeometry");
