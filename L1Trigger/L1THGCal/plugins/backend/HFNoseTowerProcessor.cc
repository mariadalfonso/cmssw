...
class HFNoseTowerProcessor : public HFNoseTowerProcessorBase {
public:
  HFNoseTowerProcessor(const edm::ParameterSet& conf) : HFNoseTowerProcessorBase(conf) {
  ...
  void run(const std::pair<edm::Handle<l1t::HGCalTowerMapBxCollection>, edm::Handle<l1t::HGCalClusterBxCollection>>&
               collHandle,
	   l1t::HFNoseTowerBxCollection& collHFNoseTowers,
    ...
    towermap3D_->buildTowerMap3D(towerMapsPtrs, collHFNoseTowers);
  }
...
DEFINE_EDM_PLUGIN(HFNoseTowerFactory, HFNoseTowerProcessor, "HFNoseTowerProcessor");
