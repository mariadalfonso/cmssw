...
HGCalTriggerGeometryESProducer::HGCalTriggerGeometryESProducer(
    ...
    : geometry_config_(iConfig.getParameterSet("TriggerGeometry")),
      geometry_name_(
          geometry_config_.getParameter<std::string>("TriggerGeometryName")),
  ...
  if (isV9Geometry_) {
    cc.setConsumes(ee_geometry_token_, edm::ESInputTag{"", "HGCalEESensitive"})
        .setConsumes(hsi_geometry_token_,
                     edm::ESInputTag{"", "HGCalHESiliconSensitive"})
        .setConsumes(hsc_geometry_token_,
                     edm::ESInputTag{"", "HGCalHEScintillatorSensitive"})
        .setConsumes(nose_geometry_token_,
...
HGCalTriggerGeometryESProducer::produce(const CaloGeometryRecord& iRecord) {
  // using namespace edm::es;
  ReturnType geometry(HGCalTriggerGeometryFactory::get()->create(
      geometry_name_, geometry_config_));
  if (isV9Geometry_) {
    // Initialize trigger geometry for V9 HGCAL geometry
    geometry->initialize(
        &iRecord.get(ee_geometry_token_), &iRecord.get(hsi_geometry_token_),
        &iRecord.get(hsc_geometry_token_), &iRecord.get(nose_geometry_token_));
  } else {
    ...
    if (not(calo_geometry.getSubdetectorGeometry(DetId::Forward, HGCEE) &&
      ...
      throw cms::Exception("LogicError")
          << "Configuration asked for non-V9 geometry, but the CaloGeometry "
