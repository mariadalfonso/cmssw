...
class HFNoseTowerProducer : public edm::stream::EDProducer<> {
public:
  HFNoseTowerProducer(const edm::ParameterSet&);
  ~HFNoseTowerProducer() override {}
...
private:
  ...
  std::unique_ptr<HFNoseTowerProcessorBase> towersProcess_;
};

DEFINE_FWK_MODULE(HFNoseTowerProducer);

HFNoseTowerProducer::HFNoseTowerProducer(const edm::ParameterSet& conf)
  ...
  towersProcess_ = HFNoseTowerFactory::get()->create(towerProcessorName, towerParamConfig);

  produces<l1t::HFNoseTowerBxCollection>(towersProcess_->name());
}

void HFNoseTowerProducer::beginRun(const edm::Run& /*run*/, const edm::EventSetup& es) {
...
void HFNoseTowerProducer::produce(edm::Event& e, const edm::EventSetup& es) {
  // Output collections
  auto towers_output = std::make_unique<l1t::HFNoseTowerBxCollection>();
