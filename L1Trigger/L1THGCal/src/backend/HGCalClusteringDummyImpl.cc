...
HGCalClusteringDummyImpl::HGCalClusteringDummyImpl(
  ...
  edm::LogInfo("HGCalClusterParameters")
      << "C2d global calibration factor: " << calibSF_;
}

// Create one cluster per TC for direct TC->3D clustering
void HGCalClusteringDummyImpl::clusterizeDummy(
    const std::vector<edm::Ptr<l1t::HGCalTriggerCell>>& triggerCellsPtrs,
    l1t::HGCalClusterBxCollection& clusters) {
  std::vector<l1t::HGCalCluster> clustersTmp;
  for (std::vector<edm::Ptr<l1t::HGCalTriggerCell>>::const_iterator tc =
           triggerCellsPtrs.begin();
       tc != triggerCellsPtrs.end(); ++tc) {
